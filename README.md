...
一套完整的免杀解决方案，涵盖 Shellcode 处理与 Loader 编写，结合 IPv4 混淆、XOR 加密、Module Stomping、WriteProcessMemory 写入等技术，用于绕过主流杀软和 EDR 检测。
...
针对 v4.0 在沙箱检测中暴露的两个行为特征进行了优化：

### 1. 消除"代码生成/载荷解密执行"高危特征

**v4.0 做法：**
```
VirtualProtect(.text → RW)  →  memcpy  →  VirtualProtect(.text → RX)
```
两次显式 `VirtualProtect` 调用，尤其是翻转为 `PAGE_EXECUTE_READ`，被沙箱标记为"调整非系统常规内存访问权限为只读可执行"。

**v4.1 改进：**
```
WriteProcessMemory(.text)  →  FlushInstructionCache
```
`WriteProcessMemory` 在内核态内部临时切换保护属性完成写入，写完后自动恢复原属性，用户态不出现显式的 `VirtualProtect(RX)` 调用。

### 2. 消除"反逆向工程(PAGE_GUARD)"可疑特征

**v4.0 做法：**
```c
ConvertThreadToFiber(NULL);
CreateFiber(0, entry, NULL);   // 内部创建 PAGE_GUARD 内存页
SwitchToFiber(sc);
EnumSystemLocalesW(entry, 0);  // 回调API
```
Fiber 机制会在内部创建 `PAGE_GUARD` 属性的内存页用于栈保护，被沙箱标记为"反逆向和反调试"。
...
entry();   // 直接函数指针调用
```
移除了全部 Fiber API 和回调 API，改为最简单的直接函数指针调用，不产生任何特殊内存属性。

### 改进对比

| 维度 | v4.0 | v4.1 |
|------|------|------|
| 代码写入 | VirtualProtect + memcpy + VirtualProtect | WriteProcessMemory |
| 执行方式 | CreateFiber + EnumSystemLocalesW | 直接函数指针调用 |
| 沙箱高危特征 | VirtualProtect(RX) 权限翻转 | 已消除 |
| 沙箱可疑特征 | PAGE_GUARD 内存页 | 已消除 |
...
└── scripts/
    ...
    └── loader_v4.c                    # Module Stomping Loader
```
...
程序启动 (main)
  ...
  解析 PE 找到 .text 段
      │
      ▼
  WriteProcessMemory(.text)        ← 内核态写入 (无显式 VirtualProtect)
      │
      ▼
  ...
  SecureZeroMemory + HeapFree      ← 擦除堆中明文
      │
      ▼
  函数指针调用(.text)               ← 直接调用执行 (无 Fiber, 无回调)
```
...
Loader 在加载签名 DLL 时按顺序尝试，确保可用性：

```
msimg32.dll  →  dcomp.dll  →  dwmapi.dll
```

每个 DLL 加载后检查 `.text` 段大小是否足够容纳 Shellcode，不足则释放后尝试下一个。
...
输出：`shellcode_obfuscated_ipv4.c`（C 语言 IPv4 数组格式，每行5个IP）
